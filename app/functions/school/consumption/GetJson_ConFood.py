...
from app.models import db, con_food_24h, con_food_12m, con_food_7d


def GetJson_ConFood(modeTime=0):
    if modeTime == 0:
        dbQuery = db.session.query(con_food_24h.sum_amount).order_by(con_food_24h.id)
        axisLabels = []
        for i in range(24):
        ...
        dbQuery = db.session.query(con_food_7d.sum_amount).order_by(con_food_7d.id)

    elif modeTime == 2:
        axisLabels = [u'一月', u'二月', u'三月', u'四月', u'五月', u'六月', u'七月', u'八月', u'九月', u'十月', u'十一月', u'十二月', ]
        dbQuery = db.session.query(con_food_12m.sum_amount).order_by(con_food_12m.id)
