...
from app.models import db, acrec, ac_loc
...
def GetJson_AcDateTrend(userID, startDate, endDate, modeDate):
    ...
    """
    dbQuery = db.session.query(acrec.ac_datetime, ac_loc.category).filter(
    ...
    # 包装成Echarts需要的格式
    if int(modeDate) == 2:
        axisLabels = list(map(lambda x: x.strftime('%Y-%m'), df.index.tolist()))
    else:
        axisLabels = list(map(lambda x: x.strftime('%Y-%m-%d'), df.index.tolist()))  # 从dataframe 中取出作为索引的日期标签成为队列
    seriesData = []
