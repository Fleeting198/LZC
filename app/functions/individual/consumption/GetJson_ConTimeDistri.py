...
def GetJson_ConTimeDistri(userID, startDate, endDate):
    ...
    """
    # Query
    dbQuery = db.session.query(consumption.con_datetime, dev_loc.category, consumption.amount).filter(
    ...
    for colName, col in df.items():
        df = list(map(lambda x: 0.0 if isnan(x) else float(x), col.tolist()))
