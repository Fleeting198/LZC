...
def GetJson_ConWater(modeTime):
    if modeTime == 0:
        strQuery = db.session.query(con_water_24h.con_axis, con_water_24h.sum_amount).order_by(con_water_24h.con_axis)
        axisLabels = []
        for i in range(24):
        ...
        strQuery = db.session.query(con_water_7d.con_axis, con_water_7d.sum_amount).order_by(con_water_7d.con_axis)
